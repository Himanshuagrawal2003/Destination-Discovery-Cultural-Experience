...
export default function Events() {
  ...
  const [cityInput, setCityInput] = useState(''); // Local state for input field to prevent immediate keystroke queries
  ...
  // Sync cityInput when citySearch is updated (like from dropdown or clear)
  useEffect(() => {
    setCityInput(citySearch);
  }, [citySearch]);

  // Reset page when filters change
  useEffect(() => {
    setPage(1);
  }, [activeType, citySearch]);
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        if (citySearch) queryParams.set('location.city', citySearch.trim());
        queryParams.set('page', String(page));
        queryParams.set('limit', '9'); // 3x3 layout fits 9 elements cleanly
    ...
    fetchEvents();
  }, [activeType, citySearch, page, user]);
  ...
  return (
    <div className="container-cq py-8 space-y-8 min-h-screen bg-[#FAF7FF] dark:bg-dark-bg">
      ...
      {/* City wise Search Form */}
      <form onSubmit={handleSearchSubmit} className="bg-white dark:bg-dark-card p-3 rounded-2xl border border-primary-100 dark:border-dark-border shadow-sm flex items-center gap-2 max-w-xl mx-auto transition-all hover:shadow-md focus-within:ring-2 focus-within:ring-accent/30 focus-within:border-accent">
        <div className="relative flex-grow flex items-center">
          <LuMapPin className="absolute left-3 text-accent text-lg" />
          <input
            type="text"
            placeholder="Search events by city (e.g. Vrindavan, Varanasi, Kyoto)..."
            value={cityInput}
            onChange={(e) => setCityInput(e.target.value)}
            className="w-full pl-9 pr-10 py-2.5 text-xs font-semibold bg-transparent border-none focus:outline-none focus:ring-0 text-primary-900 dark:text-white placeholder-primary-300 dark:placeholder-dark-muted"
          />
          {cityInput && (
            <button 
              type="button"
        ...
        <button 
          type="submit"
          className="btn bg-accent hover:bg-accent/90 text-white font-bold px-6 py-2.5 rounded-xl text-xs shadow-sm hover:shadow-glow transition-all cursor-pointer flex items-center gap-1.5 shrink-0"
        >
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-full shrink-0 border cursor-pointer transition-all ${
              activeType === type.value
                ...
                : 'bg-white dark:bg-dark-card text-primary-900/70 dark:text-dark-muted border-primary-200 dark:border-dark-border hover:bg-primary-50 dark:hover:bg-primary-955/20'
            }`}
          >
            {type.label}
          </button>
